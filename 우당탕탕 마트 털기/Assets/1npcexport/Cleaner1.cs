...
public class Cleaner1 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            transform.forward = new Vector3(0, 0, -1);
            position.z -= 0.1f * Time.deltaTime;
    ...
    public void OnTriggerEnter(Collider collision)
    {
        ...
        if (collision.GetComponent<Collider>().gameObject.CompareTag("Player"))
        {
            ...
            if(collision.gameObject.name == "Cube_front"||collision.gameObject.name == "Cube_back"){
                if(GameObject.Find("Cube_front").transform.localPosition.z - GameObject.Find("Cube_front").transform.localPosition.z > 0){print(1); recent = 2;}
                else{print(2); recent = 1;}
            }
